...
firstw=Tk()
firstw.title("Ramkrishna Mission (C.B.S.E.) ")
firstw.geometry("1600x1000+0+0")
label=Label(text="Ramkrishna Mission (C.B.S.E.) ",font=("times new roman",35),bg="MediumOrchid2")
...
def second():
    global secondw
    secondw=Tk()
    secondw.title("Ramkrishna Mission (C.B.S.E.) ")
    ...
    def distroy5():
        secondw.destroy()
        window()
    mainlabel= Label(secondw,text="Ramkrishna Mission (C.B.S.E.) ", font=("times new roman", 35), bg="MediumOrchid2")
    mainlabel.pack(side=TOP, fill=X)
...
def distroy():
    firstw.destroy()
def login():
    if user.get()=="CS" and user3.get()=="PYTHON":
        second()
        distroy()
...
def root():
    ...
    root.title("Ramkrishna Mission (C.B.S.E.) ")
    global entry1
    global entry2
    global entry3
    global entry4
    ...
    label4=Label(root,text="TOTEL FEE", font=("arial",17))
...
with conn:
    cur=conn.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS kistar(id INTEGER primary key autoincrement ,name text,sur text,email, branch text,gender text, fee int, python int,java int,c int,d int)')
...
def window():
  ...
  label4=Label(main,text="FEES YOU WANT TO PAY", font=("arial",17))
  label4.place(x=650, y=100)
...
def calculation():
 ...
 print(GENDER)
 CALCULATE = calculate.get()
 if NAME==("") and  SUR==("")and  EMAIL==("") and BOX==("SELECT") and  GENDER==(3) and  JAVA==(0) and  PYTHON==(0) and C==(0) and  D==(0):
     ...
     if box.get()=="COMPUTER SCIENCE" and gender.get()==1:
         ...
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="MECHANICAL" and gender.get()==1:
         x=(calculation2)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="MECHANICAL" and gender.get()==0:
         x=(calculation2-calculation2*10/100)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
     ...
     if box.get()=="CIVIL" and gender.get()==1:
         x=(calculation2)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="CIVIL" and gender.get()==0:
         x=(calculation2-calculation2*10/100)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
...
def msg():
 if branch.get()=="SELECT" or  gender.get()==3 or  ( python.get()==0 and  java.get==0 and c.get()==0 and d.get()==0):
 ...
 if  "@" and ".com" not in entry3.get() :
    ...
    if msg>0:
     ...
     with conn:
         cur=conn.cursor()
         cur.execute('INSERT INTO kistar (name,sur, email, branch, gender,fee ,python,java,c,d ) VALUES(?,?,?,?,?,?,?,?,?,?)',(NAME,SUR,EMAIL,BRANCH,GENDER,CALCULATE,PYTHON,JAVA,C,D,))
...
def search_gender():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT gender FROM kistar WHERE id=?',(SEARCH,))
...
def update():
    ...
    if box1>0:
     ...
     with conn:
         ...
         cur.execute('UPDATE kistar SET gender=? WHERE id=?',(GENDER,SEARCH,))
